...
(function(ViewEncapsulation3) {
  ...
  ViewEncapsulation3[ViewEncapsulation3["IsolatedShadowDom"] = 4] = "IsolatedShadowDom";
...
function createRootTView(rootSelectorOrNode, componentDef, componentBindings, directives) {
  const tAttributes = rootSelectorOrNode ? ["ng-version", "21.0.0-next.7"] : (
...
function ɵɵcontrolCreate() {
  ...
  if (customControl) {
    ...
    const tagName = tNode.value;
    throw new RuntimeError(318, `'<${tagName}>' is an invalid control host. The host must be a native form control (such as <input>', '<select>', or '<textarea>') or a custom form control component with a 'value' or 'checked' model.`);
  }
  control.register();
}
...
function updateNativeControl(tNode, lView, control) {
  const input2 = getNativeByTNode(tNode, lView);
  const renderer = lView[RENDERER];
  const state = control.state();
  setNativeControlValue(input2, state.value());
  renderer.setAttribute(input2, "name", state.name());
  setBooleanAttribute(renderer, input2, "disable", state.disabled());
  setBooleanAttribute(renderer, input2, "readonly", state.readonly());
  setBooleanAttribute(renderer, input2, "required", state.required());
  setOptionalAttribute(renderer, input2, "max", state.max());
  setOptionalAttribute(renderer, input2, "maxLength", state.maxLength());
  setOptionalAttribute(renderer, input2, "min", state.min());
  setOptionalAttribute(renderer, input2, "minLength", state.minLength());
}
...
var _additionalApplicationProviders = void 0;
...
var PlatformRef = class _PlatformRef {
  ...
  bootstrapModuleFactory(moduleFactory, options) {
    ...
    _additionalApplicationProviders = void 0;
...
function componentUsesShadowDomEncapsulation(lView) {
  ...
  return def?.encapsulation === ViewEncapsulation.ShadowDom || def?.encapsulation === ViewEncapsulation.IsolatedShadowDom;
}
...
export {
  ...
  setZoneProvidersForNextBootstrap,
  PlatformRef,
  createPlatform,
...
@angular/core/fesm2022/_debug_node-chunk.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-E3ONDL5G.js.map
